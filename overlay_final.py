...
tcc= (10*np.asarray(R)*pc)/(M*cs_hot)
    #print(tcc)
    #f = 0.9*(((0.1*Tcl_4*(GAMMA*kB/(mu*mp))*100)**0.3)*(M**0.6))
...
marker_style = dict(color='tab:blue', linestyle='None', marker='^',
                    markersize=30, markerfacecoloralt='tab:red', markeredgewidth=5)
filling = Line2D.fillStyles[-1]
plt.plot(X,Y,fillstyle=filling, **marker_style)#,label='Growing Clouds, M=1.0')
##############################################################
M=1.0
#R= [24.67,85.97]
    #R=500
...
plt.arrow(3.135, -0.75, 0.0, 0.15, head_width=0.2, head_length=0.3, fc='k', ec='k',width=0.02)
plt.text(3.2, -0.9, r'$\mathrm{\chi=300}$', horizontalalignment='center', verticalalignment='center', fontsize=40)

plt.arrow(1.525, -1.4, 0.0, 0.15, head_width=0.2, head_length=0.3, fc='k', ec='k',width=0.02)
plt.text(1.6, -1.6, r'$\mathrm{\chi=100}$', horizontalalignment='center', verticalalignment='center', fontsize=40)


plt.legend(loc='best', prop={'size': 35}, bbox_to_anchor=(0.61, 0.65),framealpha=0.3)
